...
- Een belegging in `S.wealth.holdings` is **één totaalbedrag in euro**: `manualValue` (huidige waarde, verplicht) en `totalCostEur` (totaal ingelegd, optioneel — alleen daarmee toont de app rendement). Bewust geen aantal × koers: de app haalt geen koersen op en het gaat om het totaal. Voer geen per-aandeel velden opnieuw in; schema 3 heeft ze verwijderd.
